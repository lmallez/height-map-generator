...
from entities.chunk_basic import ChunkBasic
    ...
    def get_diamond_chunk(self, pad):
        ...
        for y in range(0, size):
            for x in range(0, size):
                if (
                    self.map[y * pad][x * pad] is not None
                    and self.map[y * pad + pad][x * pad + pad] is None
                ):
                    self.map[y * pad + pad][x * pad + pad] = self.gen_circle_point(
                        x * pad + pad, y * pad + pad, pad=pad
                    )
        return
        ...
        for y in range(0, size):
            for x in range(0, size):
                if self.map[y * pad][x * pad] is None:
                    self.map[y * pad][x * pad] = self.gen_circle_point(
                        x * pad, y * pad, pad=pad
                    )
        return
