...
class PrinterMap:
    ...
    def draw_map(self, world_map, hei=True, hea=True):
        chunk_size = 1000 / max(
            world_map.size_map.y.x - world_map.size_map.x.x,
            world_map.size_map.y.y - world_map.size_map.x.y,
        )
        for pos, chunk in world_map.map.items():
            chunk_pos = [
                chunk.pos.x - world_map.size_map.x.x,
                chunk.pos.y - world_map.size_map.x.y,
            ]
            chunk_pos = Point(
                int(chunk_size) * chunk_pos[0]
                - chunk.pos.x * chunk_size / world_map.chunk_size,
                int(chunk_size) * chunk_pos[1]
                - chunk.pos.y * chunk_size / world_map.chunk_size,
            )
